...
import { revenueGross, revenueGrossPrabayar } from '../db/schema/v_honai_puma';

export const revenueGrossRouter = router({
    getAll: publicProcedure
        ...
        .query(async ({ input }) => {
            ...
            const revenueCVM = db
                .select()
                .from(revenueGross)
                .where(eq(revenueGross.transactionDate, format(selectedDate, 'yyyy-MM-dd')))
                .prepare()
    ...
    getPrabayar: publicProcedure
        .input(z.object({ date: z.coerce.date().optional() }))
        .query(async ({ input }) => {
            const { date } = input
            const selectedDate = date ? new Date(date) : subDays(new Date(), 3)

            const revenueGross = db
                .select()
                .from(revenueGrossPrabayar)
                .where(eq(revenueGrossPrabayar.transactionDate, format(selectedDate, 'yyyy-MM-dd')))
                .prepare()

            const [revenues] = await Promise.all([
                revenueGross.execute()
            ])
